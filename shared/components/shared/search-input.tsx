...
export const SearchInput: React.FC<Props> = ({className, categories}) => {
    ...
    return (
        <>
            {focused && (
                <div
                    className="fixed top-0 left-0 bottom-0 right-0 bg-black/50 z-30 pointer-events-none"
            ...
            <div
                ...
                {products.length > 0 && (
                    <div
                        ...
                        {categories?.length && categories.slice(0, 6).map(({name, id, products}, index) => (
                            ...
                            <Link
                                ...
                                href={`/#${name}`}>
                                <img className="rounded-sm h-8 w-8" src={products[0].imageUrl} alt={name}/>
                                <span>{name}</span>
                            </Link>
                        ))}
                    </div>
